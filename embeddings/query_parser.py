...
This module extracts keyword-based hints for price level, dietary needs,
and location from natural language user queries. It is intentionally simple,
deterministic, and case-insensitive so it can augment retrieval filters
without introducing model dependencies.
"""
...
def _load_default_location_keywords() -> dict:
    ...
    This is intentionally fault-tolerant. If no file is present or parsing fails,
    an empty dictionary is returned so parsing continues without location matches.
    """
...
def _extract_price_label(normalized_query: str, raw_query: str) -> str | None:
    ...
    # Handle dollar-sign shorthand before text aliases.
    if "$$$$" in raw_query:
        return "luxury"
    if "$$$" in raw_query:
        return "expensive"
...
def _extract_distance_time_intent(normalized_query: str, raw_query: str) -> dict[str, Any]:
...
def parse_query(query: str) -> dict[str, Any]:
    ...
    Returns:
        A dictionary with extracted query signals:
        - "price": one of {"cheap", "moderate", "expensive", "luxury", "unknown"} or None
        - "dietary": list[str] of matched dietary tags
        - "location": canonical location string or None
        - "occasion_vibe": list[str] canonical tags for occasion/vibe intent
        - "distance_time_intent": dict with near_me, max_minutes, and max_km
        - "meal_context": list[str] canonical meal-time tags
    """
    ...
    return {
        ...
        "distance_time_intent": _extract_distance_time_intent(normalized_query, raw_query),
